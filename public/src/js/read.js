	...
	function initPage(){
		...
		$('p.quoted').click(function(){
			var obj = $(this);
			if(!obj.hasClass('open')){
				var v = notes[obj.attr('note-index')];
				originalHTML.push(obj.html());
				obj.html(v.quote).addClass('open').append('<p class="text">'+v.text+'</p><div class="close"	 onclick="closeQuote(\''+v.element+'\','+(originalHTML.length - 1)+')"><i class="fa fa-times"></i> FECHAR</div>');
			}
		});
	}
 ...
 $('.mdlNotes .btn.red').click(function(){
  ...
  $.ajax({
     ...
     success: function (data, textStatus, jqXHR) {
        $('#note_text').val('');
        $(selectedElement).addClass('quoted');
    }
});
});
